...
function serve(handler: (hit: Hit, count: number) => Response | Promise<Response>): Mock {
  ...
  const server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      const hit: Hit = { path: new URL(req.url).pathname, body };
      hits.push(hit);
      allBodies.push(body);
...
test('happy path: exact request body + full golden-fixture mapping', async () => {
  ...
  expect(result.errorCodes).toBeUndefined(); // absent from the response → undefined, as-is
...
test('exhausted retries (always 500, non-JSON body) → success:false request-failed', async () => {
  ...
  expect(result).toEqual({ success: false, errorCodes: ['request-failed'] });
});
...
test('failure response parsing: invalid-input-response', async () => {
  ...
  expect(result.reasons).toBeUndefined(); // absent from the response → undefined, as-is
});
